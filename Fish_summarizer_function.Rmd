...
- LWfile -- the csv file containing the fish life history parameters.
  - *NOTE: this function will not work if there are species missing LW parameters*
...
for (i in 1:n) { # where n = length(fish[,1])
  if (fish$Data_Type[i] == "QUAN") {
    ...
    numbers_species[jj, spp] <- numbers_species[jj, spp] + (convert * fish$Number[i])
    biomass_species[jj, spp] <- biomass_species[jj, spp] + (convert * (fish$Number[i] * pc_biomass[i]))
...
###### NOTE TO SELF: THIS WILL NEED TO CHANGE FOR PUBLICATION AS CODE IS NOT A COLUMN #####
...
summary_group$Trophic_group <- factor(summary_group$Trophic_group, ordered = TRUE, levels = c("Top-predator shark", "Top-predator", "Lower-carnivore", "Planktivore", "Herbivore"))
